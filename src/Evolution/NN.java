...
public class NN implements Serializable {
    ...
    public static NN randomGenome() {
        ...
        for (int i : new int[]{4, 5, 6, 7}) {
            ...
            for (synapse s : NN.synapses)
                if (s.to.equals(NN.nodes.get(i + NeuralNet.inputNum))) {
                    found = true;
                    break;
                }
            if (!found) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (Math.random() < NeuralNet.mutationWeightShiftProbability * 3)
                NN.shiftWeights(NeuralNet.mutationWeightShiftStrength);
            if (Math.random() < NeuralNet.mutationWeightRandomProbability * 3)
                NN.randomWeights(NeuralNet.mutationWeightRandomStrength);
            if (Math.random() < NeuralNet.mutationBiasShiftProbability * 3)
                NN.shiftBias(NeuralNet.mutationBiasShiftStrength);
    ...
    public static void calculateWeightedOutput(NN NN, double... input) {
        ...
        while (!scanLayer.isEmpty()) {
            ...
            if (NeuralNet.batchNormalizeHiddenLayers)
