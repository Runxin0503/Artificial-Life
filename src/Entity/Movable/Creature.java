...
//    private boolean canSee;
    ...
    public void tick(ArrayList<ArrayList<ArrayList<Entity>>> vision, ArrayList<ArrayList<ArrayList<Entity>>> interaction, double[] output,World world) {
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 1"+this);
        if (++internalClockCountdown >= 60*WorldConstants.Settings.ticksToSecond) internalClockCountdown = 0;
        starving = energy == 0;
        if (!starving) maturity++;

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 2"+this);
        //see
        ArrayList<Creature> boids = new ArrayList<Creature>();
        see(vision, boids);

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 3"+this);
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 4"+this);
        if (!starving && health < maxHealth && output[5] > 0.5) deltaEnergy -= regenHealth();

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 5"+this);
        //grow size
        if (!starving) deltaEnergy -= growSize();

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 6"+this);
        //interact with entity (Eat or Attack)
        eating = output[6] > 0.5;
        if (eating) eat(interaction);

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 7"+this);
        //digest food
        deltaEnergy += digest(output[7]);

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 8"+this);
        //update metabolism
        baseEnergyCost = Energy.energyCostFormula(stomachFluid, size, strengthAvailable, brain.nodes.size(), brain.synapses.size());

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 9"+this);
        //update stomach size
        stomachSize = Stomach.sizeToStomachSize(size);

        if(!Double.isFinite(energy))System.out.println("ERROR GATE step 10"+this);
        //reset clock
        if (output[10] > 0.5) internalClockCountdown = 0;

        //add delta energy
        if(!Double.isFinite(deltaEnergy))System.out.println("ERROR GATE ID 4"+this);
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE ID 5??"+this);
        if(!isPlayer) if(health <= 0 || maturity < 0 || size < Reproduce.babySize || !Double.isFinite(energy) || maturity >= Reproduce.maturityDeath){
            death(world);
            return;
        }
    ...
    public void thinkThonk(){
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE BRAIN step 2"+this);
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE BRAIN step 3"+this);
    }
    private double updateVel(double moveForward,double moveBackward,double turnLeft,double turnRight,double herdYes,double herdNo,ArrayList<Creature> boids){
        ...
        if(((herdNo>0.5&&!boids.isEmpty())||herdYes>0.5)){
            ...
            if(deltaTheta<0!=angularSpeed<0||Math.abs(deltaTheta)>Boids.angleAdjustmentThreshold){
                ...
                if(deltaTheta>0&&angularSpeed<deltaTheta) {}//keep accelerating positive
    ...
    private Vector2D getHerdingVector(ArrayList<Creature> boids,boolean positiveStimuli){
        if(boids.isEmpty())return new Vector2D(Math.cos(direction)*Boids.minHerdingSpeed*forceAvailable,Math.sin(direction)*Boids.minHerdingSpeed*forceAvailable);
    ...
    private void eat(ArrayList<ArrayList<ArrayList<Entity>>> nearbyEntities) {
        Rectangle eatingHitbox = getEatingHitbox();

        for(ArrayList<ArrayList<Entity>> types : nearbyEntities) for(ArrayList<Entity> grid : types)for(Entity e : grid)if(!e.equals(this) && e.getBoundingBox().intersects(eatingHitbox)){
...
//            }
    ...
    private void death(World world){
        ...
        if(!Double.isFinite(energy))System.out.println("ERROR GATE ID 3 "+this);
    ...
    //accessor functions
    @Override
    public double getMass(){return size * Math.sqrt(size) * Movement.sizeMovementConstant * genome.armourMultiplier;}
    ...
    public CreatureGenome getGenome(){return genome;}
    public double getMaxHealth(){return maxHealth;}
    public double getArmour() {
        return armourAvailable;
    }
    public double getAngularSpeed(){return angularSpeed;}
    public double getMetabolism(){return deltaEnergy;}
    ...
    public double getPlantMass(){return plantMass;}
    public double getMeatMass(){return meatMass;}
    public double getStomachFluid(){return stomachFluid;}
    public double getStomachSize(){return stomachSize;}
    ...
    public double[] getReport(){
        ...
        for(int i=0;i<report.length;i++) if(!Double.isFinite(report[i])){
