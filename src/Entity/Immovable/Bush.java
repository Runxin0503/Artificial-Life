...
public class Bush extends Immovable implements Serializable {
    ...
    public Bush(int x, int y) {
        ...
        for (int i = 0; i < currentBerries; i++)
            berries.add(new Point((int) (Math.random() * (getBoundingBox().width - ImageConstants.berriesWidth)), (int) (Math.random() * (getBoundingBox().height - ImageConstants.berriesHeight))));
    ...
    private void clearQueue() {
        ...
        while (!berries.isEmpty() && !queuedBerryEating.isEmpty()) {
            ...
            if (c.addPlantMass(BushConstants.energy / CreatureConstants.Stomach.plantMassToEnergy))
