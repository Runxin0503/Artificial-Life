...
public class globalNodes {
    ...
    public globalNodes() {
        ...
        for (int i = 0; i < inputNum; i++) primitiveNodes.add(new node(Type.input, i, 0.1, (i + 1) / (inputNum + 1.0)));
        for (int i = inputNum; i < inputNum + outputNum; i++)
            primitiveNodes.add(new node(Type.output, i, 0.9, (i + 1 - inputNum) / (outputNum + 1.0), outputAF));
    }
    ...
    public node add(node from, node to) {
        node n = new node(Type.hidden, primitiveNodes.size(), (from.x + to.x) / 2, (from.y + to.y) / 2 + Math.random() * 0.1 - 0.05, hiddenAF);
